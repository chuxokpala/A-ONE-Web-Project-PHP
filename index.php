<?php require('includes/header.php')?>

    <!-- Hero Section -->
    <section class="hero">
    <div class="slider" id="slider">
        <div class="slide active" style="background-image: url('./assets/web images/Home Page x1.jpg');">
            <h1>A-ONE Electric & Solar</h1>
            <p>Power & Energy Services</p>
            <p>“Proven expertise with project delivery”</p>
        </div>
        <div class="slide" style="background-image: url('./assets/web images/Home Page x2.jpg');">
            <h1>Renewable Energy</h1>
            <p>Powering Sustainable Futures</p>
            <p>“From homes to industries, we design and build solar solutions for a cleaner, resilient tomorrow.”</p>
        </div>
        <div class="slide" style="background-image: url('./assets/web images/Home Page x3.jpg');">
            <h1>Power Infrastructure</h1>
            <p>Bridging Energy Gaps</p>
            <p>“Delivering reliable power and electrification solutions to light up communities and drive progress.”</p>
        </div>
        <div class="slide" style="background-image: url('./assets/web images/Home Page x4.jpg');">
            <h1>Consulting Services</h1>
            <p>Empowering Informed Decisions</p>
            <p>“Providing expert insights into energy projects to accelerate the energy transition.”</p>
        </div>
    </div>
</section>

    
    <!-- Welcome Message -->
    <section class="welcome" id="welcome">
        <div class="welcom-mssg">
            <h3>Welcome to AESL</h3>               
            <p>AESL is a energy project delivery, advisory, investment and capacity development firm. With our expert team and best practices, we deliver services in the energy 
                and electricity industry in a sustainable way, without compromising the livelihood of future generations. We 
                specialise in the delivery of solar energy systems (of all sizes and purposes), and with our team of engineers and strategic 
                collaborations, we deliver projects and services across the energy value-chain....</p>

                <p>We welcome private individuals, businesses, NGOs and government clients looking to execute sustainable initiatives and projects.</p>
        </div>
    </section>
    
    <!-- Our Services -->
    <section id="services">
        <!-- Section Heading -->
        <div class="section-heading">
            <h3>Our Services</h3>
            <p>Explore our range of services designed to optimize efficiency, sustainability, and performance in the energy and infrastructure sectors.</p>
        </div>
    
        <!-- Services Cards Container -->
        <div class="services-container">

            <!-- Project Delivery -->
            <div class="service-card">
                <img src="./assets\web images\Project Management x.png" alt="Project Development" class="service-image">
                <h3 class="service-title">Project Delivery</h3>
                <p class="service-description">
                    We develop and implement end-to-end energy systems including SHS, solar mini-grids, and conventional electric power infrastructure with a focus on quality, reliability, and speed.
                </p>
                <a href="contact.php" class="read-more">Contact Us</a>
            </div>  
            
             <!-- Strategic Consulting -->
            <div class="service-card">
                <img src="./assets\web images\Consulting x.png" alt="Strategic Consulting" class="service-image">
                <h3 class="service-title">Energy Advisory</h3>
                <p class="service-description">We provide strategic guidance on system optimization, regulatory compliance, feasibility studies, and commercial agreements, enabling clients to make informed, efficient energy decisions.</p>
                <a href="contact.php" class="read-more">Contact Us</a>
            </div>

             <!-- Financial Advisory -->
            <div class="service-card">
                <img src="./assets\web images\Financial Services.png" alt="Financial Advisory" class="service-image">
                <h3 class="service-title">Investments</h3>
                <p style="margin-bottom: 40px" class="service-description">We invest in and co-develop clean energy projects, bringing both capital and technical expertise to scale viable, bankable and impactful solutions.</p>
                <a  href="contact.php" class="read-more">Contact Us</a>
            </div>

            <!-- Energy Solutions -->
            <div class="service-card">
                <img src="./assets\web images\Energy Solutions xa.png" alt="Energy Solutions" class="service-image">
                <h3 class="service-title">Capacity Development</h3>
                <p class="service-description">We offer technical training and broader capacity-building programs to equip individuals and institutions with the skills to operate, manage, and sustain energy systems</p>
                <a href="contact.php" class="read-more">Contact Us</a>
            </div> 
        </div>
    </section>
    
    <!-- Client Section -->
    <section class="client" id="client">
        <div class="client-container">
            <h3>Our Clients</h3>
            <p>We are proud to have worked with a range of clients across different sectors.</p>

            <div class="client-logos-wrapper">
            <div class="client-logos">
            <img src="./assets\web images\ICRC Logo.png" alt="Our Clients">
            <img src="./assets\web images\NCS Logo.png" alt="Our Clients">
            <img src="./assets\web images\NDDC Logo.jpeg" alt="Our Clients">
            <img src="./assets\web images\WHO Logo.png" alt="Our Client">
            <img src="./assets\web images\ICRC Logo.png" alt="Our Clients">
            <img src="./assets\web images\NCS Logo.png" alt="Our Clients">
            <img src="./assets\web images\NDDC Logo.jpeg" alt="Our Clients">
            <img src="./assets\web images\ICRC Logo.png" alt="Our Clients">
            <img src="./assets\web images\WHO Logo.png" alt="Our Client">
        </div>
        </div>
        </div>
     </section>


     <!-- load your Javascript file here -->
      <script src="js/index.js"></script>
  <?php
  require('includes/footer.php')

  ?>